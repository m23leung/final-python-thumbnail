...
def upload_to_s3(bucket, key, image, img_size):
  ...
  response = s3.put_object(ACL="public-read", Body=out_thumbnail, Bucket=bucket, ContentType="image/png",Key=key)
...
def s3_delete_item(event, context):
  ...
  if response['ResponseMetadata']['HTTPStatusCode'] == 200:
    response = {
      "statusCode": 200,
      "headers": { 'Content-Type': 'application/json', 'Access-Control-All-Origin': '*'},
